...
void Human::initPlayer(int gSize,char *FileName,int frms)
{
    gridSize = gSize;
    frames = frms;
...
void Human::placePlayer(int x, int y)
{
    plyLoc.x =  converter(x,y).x;
    plyLoc.y =  converter(x,y).y;
}

void Human::movePlayer(char* dir, int frames)
{
   /*
   if(steps >= unitWidth) {
...
loc Human::converter(int x, int y)
{
  loc val;
        x+=1;
        y+=1;
...
tuple<int,string*,int*, int*> Human::shortestPath(Graph* G,int startloc,int tarloc,int num){ //return the number of steps, travel direction, and grid spaces that character will travel
...
int Human::Dmat(char a, char b) {    //INEFFICIENT, IMPLEMENT HASH TABLE           //return the distance between two terrains
