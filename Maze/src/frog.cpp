...
void Frog::initFrog(int gSize,char *FileName,int frms)
{
    gridSize = gSize;
    frames = frms;
...
void Frog::placeFrog(int x, int y)
{
    plyLoc.x =  converter(x,y).x;
    plyLoc.y =  converter(x,y).y;
}

void Frog::moveFrog(char* dir, int frames)
{
   /*
   if(steps >= unitWidth) {
   placeFrog(getFrogLoc().x,getFrogLoc().y);
   steps =0;
   }
   */
...
loc Frog::converter(int x, int y)
{
  loc val;
        x+=1;
        y+=1;
...
tuple<int,string*,int*, int*> Frog::shortestPath(Graph* G,int startloc,int tarloc,int num){ //return the number of steps, travel direction, and grid spaces that character will travel
...
int Frog::Dmat(char a, char b) {    //INEFFICIENT, IMPLEMENT HASH TABLE           //return the distance between two terrains
