...
void Fish::initFish(int gSize,char *FileName,int frms)
{
    gridSize = gSize;
    frames = frms;
...
void Fish::placeFish(int x, int y)
{
    plyLoc.x =  converter(x,y).x;
    plyLoc.y =  converter(x,y).y;
}

void Fish::moveFish(char* dir, int frames)
{
   /*
   if(steps >= unitWidth) {
   placeFish(getFishLoc().x,getFishLoc().y);
   steps =0;
   }
   */
...
loc Fish::converter(int x, int y)
{
  loc val;
        x+=1;
        y+=1;
...
tuple<int,string*,int*, int*> Fish::shortestPath(Graph* G,int startloc,int tarloc,int num){ //return the number of steps, travel direction, and grid spaces that character will travel
...
int Fish::Dmat(char a, char b) {    //INEFFICIENT, IMPLEMENT HASH TABLE           //return the distance between two terrains
