...
static void resize(int width, int height)
{
   ...
   // glFrustum(-4,4,-3,3,0.01,100);
	gluPerspective (45.0f,Ratio,0.0001f, 100.0f);
...
void mouse(int btn, int state, int x, int y){
    ...
    switch(btn){
        ...
        if(state==GLUT_DOWN){
             ...
             xpos = (x - (Wwidth/2))/scale; // Get the mouse x and y coordinate, and translate based on origin.
             ypos = ((Wheight/2) - y)/scale;

             cout<<x<<"  "<<y<<endl;
             cout<< xpos<<"  "<<ypos <<"  " << scale <<endl;
